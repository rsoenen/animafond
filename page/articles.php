...
<?php
    ...
    if(isset($_POST['confsupprimerarticle'])&&isset($_SESSION['gererArticle'])&&$_SESSION['gererArticle']==true){ //CONFIRMATION DE SUPPRESSION D'ARTICLE
        echo '<table><form action="articles.php" method="POST"><input type="hidden" name="supprimerarticle" value="'.$_POST['confsupprimerarticle'].'"/><tr><td cospan="2">Etes vous sur de supprimer cet article<br/></td></tr><tr><td><input type="submit" value="OUI"></td></form>';
        echo '<td><form action="articles.php" method="POST"><input type="submit" value="NON"/></form></td><table>';
    }
    ...
    foreach ($listArticle as $article){ // récupération des données des articles dans la base de donné, en les mettant dans des tableaux
        ...
        if(isset($_SESSION['gererArticle'])&&$_SESSION['gererArticle']==true){ //SUPPRIMER UN ARTICLE
            echo '<td width="50px"><form method="POST" action="articles.php">';
